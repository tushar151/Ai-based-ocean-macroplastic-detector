...
if uploaded_file:
    ...
    if detect_btn:
        # Convert to numpy array (still for model compatibility)
        image_np = np.array(image)
        ...
        # Convert PIL image to draw bounding boxes
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Draw bounding box and label using PIL
                draw.rectangle([x1, y1, x2, y2], outline=box_color, width=3)
                draw.text((x1, y1 - 10), f"{label} ({conf:.2f})", fill=box_color)
